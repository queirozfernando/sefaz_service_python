...
def _monta_xml_evento_cancelamento(
    uf: str,
    ...
    ev = etree.SubElement(det, "evCancMDFe")
    etree.SubElement(ev, "descEvento").text = "Cancelamento"
    etree.SubElement(ev, "nProt").text = nprot
    etree.SubElement(ev, "xJust").text = xjust
...
def sefaz_mdfe_cancelar(
    ...
    """
    Cancela um MDF-e autorizado (evento 110111).
    """
    # 1) Monta XML do evento
    xml_evento = _monta_xml_evento_cancelamento(
        uf=uf,
        ambiente=ambiente,
        ch_mdfe=chave,
        nprot=nprot,
