...
def _monta_xml_evento_encerramento(
    uf: str,
    ...
    """
    Evento 110112 – Encerramento de MDF-e.
    dtEnc é gerado automaticamente com a data atual (AAAA-MM-DD).
    cUF final é a própria UF informada.
    """
    ...
    dt_enc = date.today().strftime("%Y-%m-%d")
    ...
    ev = etree.SubElement(det, "evEncMDFe")
    etree.SubElement(ev, "descEvento").text = "Encerramento"
    etree.SubElement(ev, "nProt").text = nprot
    etree.SubElement(ev, "dtEnc").text = dt_enc
    etree.SubElement(ev, "cUF").text = cuf
    etree.SubElement(ev, "cMun").text = cMunicipio
...
def sefaz_mdfe_encerrar(
    ...
    - dtEnc = data atual.
    - cUF final = UF informada.
    """
    # 1) Monta XML do evento
    xml_evento = _monta_xml_evento_encerramento(
        uf=uf,
        ambiente=ambiente,
        ch_mdfe=chave,
        nprot=nprot,
